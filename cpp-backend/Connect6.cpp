...
void myturn(int x[], int y[], int cnt) {
  ...
  // 자식에서 필승수를 우선 찾아보자.
	...
	if(DEBUG) cout << "search depth " << search_depth << " root score : " << Tree->root->score[search_depth] << '\n';
  if (x[0] == 0x1f) {
		for (auto& ch : Tree->root->children) {
      if(DEBUG) cout << move1X(ch->myMove) << ' ' << move1Y(ch->myMove) << ' ' << move2X(ch->myMove) << ' ' << move2Y(ch->myMove) << ' ' << " : " << ch->score[search_depth-1] << '(' << score2cmp(ch->score[search_depth-1]) << ')' << '\n';		
...
bool setting(int level){
  init();
  int timeByLevel[5] = {1000, 2000, 4000, 6000, 10000};
...
void showboard(){
  cout << "   00 01 02 03 04 05 06 07 08 09 10 11 12 13 14 15 16 17 18\n";
  for(int i = 0; i < 19; i++){
    if(i<10) cout << 0;
    cout << i << ' ';
    for(int j = 0; j < 19; j++){      
...
void run(){
  while(true){
    ...
    if(command == "SETTING"){
      int level;
      cin >> level;
      if(setting(level)) cout << "SETTING OK\n";
      else cout << "SETTING FAIL\n";
    }
    // BLOCK 11 6
    if(command == "BLOCK"){
      int x, y;
      cin >> x >> y;
      if(block(x, y)) cout << "BLOCK OK\n";
      else cout << "BLOCK FAIL\n";
      if(DEBUG) showboard();
    }
    // OPMOVE 1 4 6
    if(command == "OPMOVE"){ // Human Move
      int num;
      cin >> num;
      int x[2], y[2];
      for(int i = 0; i < num; i++) cin >> x[i] >> y[i];
      if(opmove(x, y, num)) cout << "OPMOVE OK\n";
      else cout << "OPMOVE FAIL\n";
      if(DEBUG) showboard();
    }
    // MYMOVE 2
    if(command == "MYMOVE"){
      ...
      cin >> num;
      int x[2], y[2];
      if(!mymove(x, y, num)) cout << "MYMOVE FAIL\n";
      else{
        cout << "MYMOVE OK ";
        if(num == 1) cout << num << ' ' << x[0] << ' ' << y[0] << '\n';
        else cout << num << ' ' << x[0] << ' ' << y[0] << ' ' << x[1] << ' ' << y[1] << '\n';
      }
      if(DEBUG) showboard();
    }
    // QUIT
